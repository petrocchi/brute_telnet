...
int main(int argc , char *argv[]) {
	...
	for(i=0; i<NUM_T; i++) {			// inizialized struct of threads
		if((tdata[i].taddr = (struct sockaddr_in *)malloc(sizeof(addr))) == NULL) {
			perror("malloc");
			exit(1);
		}
